import React from 'react';
import PropTypes from 'prop-types';
import { Container, Row, Col, Card, Button } from 'react-bootstrap';

import PurchaseModal from './PurchaseModal';
import SellModal from './SellModal';

const sampleSrc = "data:image/png;base64,";

const createCard = (title, src, text, buttonText, buttonModalName) => {
  return { title, src, text, buttonText, buttonModalName };
}

const createItem = (id, name, price) => {
  return { id, name, price };
}

const items = [
  createItem(0, "-Pilih-", 0),
  createItem(1, "Susu", 23000),
  createItem(2, "Kerupuk", 50000),
  createItem(3, "Minyak", 11000)
];

class Transaction extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      cards: [
        createCard("Pembelian", sampleSrc, "Penambahan stok barang untuk gudang", "Beli", "purchaseModal"),
        createCard("Penjualan", sampleSrc, "Penjualan barang", "Jual", "sellModal")
      ],
      purchaseModal: {
        show: false
      },
      sellModal: {
        show: false
      }
    };
  }

  handleModalOpen = (modalName) => {
    this.setState({ [modalName]: { show: true }});
  }

  handleModalClose = (modalName) => {
    this.setState({ [modalName]: { show: false }});
  }

  render() {
    return (
      <>
        <Container fluid>
          <Row className="justify-content-md-center">
            {this.state.cards.map((card, i) => (
              <Card key={i} style={{ width: '18rem', margin: '20px 10px', marginBottom: '0px' }}>
                <Card.Img variant="top" src={card.src}/>
                <Card.Body>
                  <Card.Title>{card.title}</Card.Title>
                  <Card.Text>{card.text}</Card.Text>
                  <Button variant="primary" onClick={(e) => this.handleModalOpen(card.buttonModalName)}>
                    {card.buttonText}
                  </Button>
                </Card.Body>
              </Card>
            ))}
          </Row>
        </Container>
        {this.state.purchaseModal.show &&
          <PurchaseModal
            items={items}
            handlePurchaseModalClose={() => this.handleModalClose("purchaseModal")}
          />}
        {this.state.sellModal.show &&
          <SellModal
            items={items}
            handleSellModalClose={() => this.handleModalClose("sellModal")}
          />}
      </>
    );
  }
}

Transaction.propTypes = {}

export default Transaction;
